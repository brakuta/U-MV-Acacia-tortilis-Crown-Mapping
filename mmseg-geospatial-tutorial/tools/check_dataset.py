#!/usr/bin/env python3
"""Validate an MMSegmentation-style dataset folder (img_dir/ann_dir per split) before training.
...
def check_split(root: Path, split: str, suffix: str, sample: int, seed: int = 0) -> bool:
    ...
    if not set(mask_vals) <= {0, 1, 255}:
        ok = False
        print('   !! mask values outside {0, 1, 255}: re-encode masks as class indices (0 = background)')
    if not any(v not in (0, 255) for v in mask_vals):
        print('   !! no target-class pixels found in the sample')
    if side:
